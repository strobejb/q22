...
- Use `loading(lazy)` on `dynamic_array(...)` for large archive/file-system directory trees or other referenced collections that should only decode children when expanded. Use `loading(eager)` only when immediate child rows are deliberately needed. `loading(...)` is parsed on all dynamic tags, but lazy materialization currently applies to dynamic arrays.
...
- For archive/directory formats, keep metadata rows browseable. Put `name(...)`, diagnostics, and semantic emits on the directory/table entries users browse, but put `nested(...)` on a real or generated byte-backed payload child such as `CompressedData[]` or `FileData[]`. Do not mark the metadata record itself as `nested(...)` unless that record row is only a byte slice with no useful child fields.
- For ZIP-like formats, show both local records and directory/trailer records when both exist. Keep earlier physical record streams as small raw samples when local payload sizes can be placeholders or require expensive parsing. Use top-level offset sorting in the renderer when physical order matters.
...
[count(entryCount),
 element(name(FileName),
         dynamic_array(name(CompressedData),
                       ...
                       loading(eager),
                       nested(type(auto), offset(payloadOffset),
