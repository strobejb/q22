...
- For ZIP-like formats, show both local records and directory/trailer records when both exist, but put names, `nested(...)`, semantic emits, and diagnostics on the directory/table entries users browse. Keep earlier physical record streams as small raw samples when local payload sizes can be placeholders or require expensive parsing. Use top-level offset sorting in the renderer when physical order matters.
- For large archive/table formats, prefer exact `count(...)` values from the file's own header or directory. When the format only has "read until this marker/value" data, combine `terminated_by(...)` with a sensible `max_count(...)`, `extent(...)`, or both. Avoid giving duplicate rich views to the same entries, and profile with `QEXED_STRUCTURE_PROFILE=1` when a definition feels slow.
...
- For offsets relative to the start of a containing table/record, use `base_of(scope)` rather than reconstructing the table base from `current_offset()` and `array_index()`. `base_of(...)` returns a root/source-relative offset suitable for unqualified `offset(...)`; use `parent::parent` explicitly when the current row is an array element under an array container.
