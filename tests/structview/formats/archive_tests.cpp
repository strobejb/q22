...
void StructViewArchiveTests::builderRendersZipCentralDirectoryFromEocd()
{
    ...
    QVERIFY(findChildNamed(central->children[0].get(), QStringLiteral("dword RelativeOffsetOfLocalHeader")));
    StructureRow *firstCentralPayload = findChildNamed(central->children[0].get(), QStringLiteral("BYTE CompressedData[]"));
    QVERIFY2(firstCentralPayload, qPrintable(childNames(central->children[0].get())));
    QVERIFY(firstCentralPayload->hasOpenAsTarget);
    QCOMPARE(firstCentralPayload->openAsRootTypeName, QStringLiteral("auto"));
    QCOMPARE(firstCentralPayload->openAsName, QStringLiteral("a.txt"));
    QCOMPARE(firstCentralPayload->openAsTransform, QStringLiteral("deflate"));
    QCOMPARE(firstCentralPayload->openAsOffset, uint64_t(firstOffset + 30 + 5));
    QCOMPARE(firstCentralPayload->openAsByteLength, uint64_t(5));
    QVERIFY(!central->children[1]->hasOpenAsTarget);
    StructureRow *secondCentralPayload = findChildNamed(central->children[1].get(), QStringLiteral("BYTE CompressedData[]"));
    QVERIFY2(secondCentralPayload, qPrintable(childNames(central->children[1].get())));
    QVERIFY(secondCentralPayload->hasOpenAsTarget);
    QCOMPARE(secondCentralPayload->openAsName, QStringLiteral("b.txt"));
    QCOMPARE(secondCentralPayload->openAsTransform, QStringLiteral("deflate"));
    QCOMPARE(secondCentralPayload->openAsOffset, uint64_t(secondOffset + 30 + 5));
    QCOMPARE(secondCentralPayload->openAsByteLength, uint64_t(5));
    ...
    QVERIFY(findChildNamed(storedCentral->children[0].get(), QStringLiteral("dword RelativeOffsetOfLocalHeader")));
    StructureRow *storedCentralPayload = findChildNamed(storedCentral->children[0].get(), QStringLiteral("BYTE CompressedData[]"));
    QVERIFY2(storedCentralPayload, qPrintable(childNames(storedCentral->children[0].get())));
    QVERIFY(storedCentralPayload->hasOpenAsTarget);
    QCOMPARE(storedCentralPayload->openAsName, QStringLiteral("large.bin"));
    QCOMPARE(storedCentralPayload->openAsTransform, QString());
    QCOMPARE(storedCentralPayload->openAsOffset, uint64_t(largeOffset + 30 + 9));
    QCOMPARE(storedCentralPayload->openAsByteLength, uint64_t(5000));
}
...
void StructViewArchiveTests::builderRendersLargeZipCentralDirectoryWithoutNameLookupBlowup()
{
    ...
    QVERIFY(!central->children[0]->hasOpenAsTarget);
    StructureRow *payload = findChildNamed(central->children[0].get(), QStringLiteral("BYTE CompressedData[]"));
    QVERIFY2(payload, qPrintable(childNames(central->children[0].get())));
    QVERIFY(payload->hasOpenAsTarget);
    QCOMPARE(payload->openAsName, QString::fromLatin1(names[0]));
    QCOMPARE(payload->openAsTransform, QString());
    QCOMPARE(payload->openAsByteLength, uint64_t(1));
}
