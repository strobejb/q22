...
public:
	...
	//
	bool		init();
	bool		open(const std::string &filename, bool readonly = false, bool quickload = false);
	...
	// returns the underlying file handle (as void*) for the original file buffer,
	// or nullptr if none. Replaces the Win32 HANDLE _handle() method.
	void*		_handle();
...
private:
	...
	//
	span *			loadspan(const std::string &filename, bool readonly = false, bool keepondisk = false);
	...
	bool			saveto(void *hFile, bool quicksave);
	bool			writespan(void *hFile, size_w index, span *sptr);
...
public:
	buffer_control() : hFile(nullptr), readonly(false)
	{
	}
	...
	bool		load(const std::string &filename, bool reado, bool quickload);
	void		free();
...
private:
	...
	size_w		length;
	size_w		maxsize;
	// hFile is a QFile* stored as void* to avoid including QFile here.
	// Cast to QFile* wherever used in seqbuf.cpp.
	void	   *hFile;
	size_t		id;
	bool		ownbuf;
	bool		readonly;
