...
// Forward declarations of helpers implemented in seqbuf.cpp
...
void odebug(const char *fmt, ...)
{
	...
	vsnprintf(buf, sizeof(buf), fmt, varg);
	// OutputDebugString not available cross-platform; write to stderr instead
...
#ifdef DEBUG_SEQUENCE
	// Cross-platform timestamp for debug log filename
	{
		time_t t = time(nullptr);
		struct tm *tm_info = localtime(&t);
		char timebuf[64];
		strftime(timebuf, sizeof(timebuf), "%Y%m%d-%H%M%S", tm_info);
		snprintf(debugfile, sizeof(debugfile), "seqlog-%s.txt", timebuf);
	}
#endif
...
sequence::span * sequence::loadspan(const std::string &filename, bool readonly, bool quickload)
{
	buffer_control *bc = new buffer_control();
...
bool sequence::writespan(void *hFile, size_w index, sequence::span *sptr)
{
	...
	{
		seqchar *src;
		size_w len = std::min(copylen, (size_w)100);
...
bool sequence::saveto(void *hFile, bool quicksave)
{
	span *sptr;
	size_w spanindex;
...
bool sequence::save(const std::string &filename /* = std::string() */)
{
	void *hFile = nullptr;
	bool quicksave = true;
	std::string newname;

	// can't overwrite existing file if no file is open and no name given
	if(origfile_id == (size_t)-1 && filename.empty())
		return false;
	...
	//
	if(can_quicksave && origfile_id != (size_t)-1 && filename.empty())
	{
	...
	else
	{
		...
		else
		{
			...
			hFile = alloctmpfile(newname, sequence_length);
		}
	...
	{
		// truncate/extend file to final size
		allocfile(hFile, sequence_length);

		if(quicksave == false)
		{
			// hFile is a QFile* — close and delete it
			// (the actual close/delete is done via the helper in seqbuf.cpp)
			// We replicate the pattern: close then free
			// seqbuf.cpp provides closefile() for this purpose
			extern void closefile(void *hFile);
			closefile(hFile);
		}

		// remember the original file name before clearing
		std::string origfile = origfile_name;

		// clear the sequence — frees all buffers and closes open file handles
		clear();

		// if we had to create a temporary, move it over the original
		if(!newname.empty())
		{
			extern void movefile(const std::string &src, const std::string &dest);
			movefile(newname, origfile);
		}

		// reinitialize with the new file
		std::string openname = filename.empty() ? origfile : filename;
		return open(openname, false);
	}
...
void sequence::restore_spanrange (span_range *range, bool undo_or_redo)
{
	...
	else
	{
		...
		else
		{
			...
			// unlink the spans from the main list
...
bool sequence::undoredo (eventstack &source, eventstack &dest)
{
	...
	while(!source.empty() && (source.back()->group_id == gid && gid != 0));
...
{
	bool app = index == sequence_length ? true : false;

	if(insert_worker(index, buf, length, action_insert))
	{
		if(can_quicksave && !app)
			can_quicksave = false;
...
bool sequence::clear ()
{
	...
	// make sure to setup all the file descriptors again to 'empty'!
	init();
	return true;
}
...
size_t sequence::render(size_w index, seqchar *dest, size_t length, seqchar_info *infobuf /*= 0*/) const
{
	...
	{
		size_w copylen   = std::min((size_w)(sptr->length - spanindex), (size_w)length);
		...
		{
			seqchar *src;
			size_t len = (size_t)std::min(copylen, (size_w)100);
...
bool sequence::insert_snapshot(size_w index, size_w length, span_desc *desclist, size_t desclen)
{
	...
	if(insert(index, (seqchar *)nullptr, length))
	{
		can_quicksave = false;
...
bool sequence::replace_snapshot(size_w index, size_w length, span_desc *desclist, size_t desclen)
{
	...
	if(replace(index, (seqchar *)nullptr, length))
	{
		injectsnapshot(index, desclist, desclen);
		breakopt();
		return true;
...
size_w sequence::replace_file(const std::string &filename, size_w index, bool linkto)
{
	breakopt();
...
//	sequence::insert
//
//	insert 'count' copies of the specified character
//
bool sequence::insert (size_w index, const seqchar val, size_w count)
{
	bool ok = true;
	for(size_w i = 0; i < count && ok; i++)
		ok = insert(index + i, &val, 1);
	return ok;
}
...
//	sequence::replace
//
//	replace 'count' elements starting at index with the specified character
//
bool sequence::replace (size_w index, const seqchar val, size_w count)
{
	bool ok = true;
	for(size_w i = 0; i < count && ok; i++)
		ok = replace(index + i, &val, 1, 1);
	return ok;
}

size_w sequence::insert_file(const std::string &filename, size_w index, bool linkto)
{
	breakopt();
