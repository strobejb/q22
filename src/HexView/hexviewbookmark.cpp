...
{
    if (!m_noteEditor || !m_noteEditor->isVisible())
        return;
    ...
    const bool hadFocus = m_noteEditor->hasFocus() || m_noteEditor->viewport()->hasFocus();
    m_noteEditor->hide();
    bool changed = false;
