...
HighlightChoice chooseHighlight(const QList<Bookmark> &highlights, size_w pos, bool gap)
{
    ...
    for (int i = 0; i < highlights.size(); ++i)
    {
        ...
        if (gap ? (bm.offset >= pos) : (pos < bm.offset))
            continue;
        if (pos >= endExclusive(bm))
            continue;
        if (bm.colourIndex == -2)
        {
                                   ...
                                   const BookmarkLayoutRequest &request)
{
    ...
    auto bookmarkContainsCursor = [&](int idx)
    {
        ...
        return request.cursorOffset >= bm.offset && request.cursorOffset < endExclusive(bm);
    };
    ...
    if (request.allowStateUpdates && !request.mouseHeld)
    {
        ...
        if (cursorIdx >= 0)
        {
            ...
            else if (result.expandedBookmarkIdx >= 0 && result.expandedBookmarkIdx != cursorIdx)
            {
    ...
    for (const QVector<int> &group : groups)
    {
        ...
        if (winnerIdx < 0 && request.mouseHeld && groupContains(group, result.expandedBookmarkIdx))
        {
    ...
    auto collapsedTabOverlapsKept = [&](int idx)
    {
        for (const ActiveStrip &kept : keptActive)
        {
            if (geometry[idx].tabTop < kept.rect.bottom() && geometry[idx].tabBot > kept.rect.top())
                return true;
        }
        return false;
    };
    ...
    for (const ActiveStrip &kept : keptActive)
    {
        for (int j = 0; j < n; ++j)
        {
            if (result.layout[j].isActive || result.layout[j].hidden)
                continue;
            if (geometry[j].tabTop < kept.rect.bottom() && geometry[j].tabBot > kept.rect.top())
