...
const char * Parser::inenglish(TOKEN t, bool use_t_state)
{
	...
	if(t == TOK_IDENTIFIER)
	{
		sprintf(buf, "Identifier \'%s\'", this->t.str);
		return buf;
	}
	else if(t == TOK_STRINGBUF)
	{
		sprintf(buf, "String '%s'", this->t.str);
		return buf;
	}
	else if(t == TOK_INUMBER)
	{
		sprintf(buf, "'%d'", (int)this->t.num);
		return buf;
	}
	else if(t == TOK_FNUMBER)
	{
		sprintf(buf, "'%f'", this->t.fnum);
		return buf;
	}
	else
	{
...
void Parser::LexerError(void *context, ERROR err, va_list vargs)
{
	Parser *parser = (Parser *)context;
	parser->ErrorV(err, vargs);
}
