...
void Parser::Dump(FILE *fp)
{
	...
	if(typeLibrary->globalFileHistory.empty())
		return;

	FILE_DESC *fdesc = typeLibrary->globalFileHistory[0];
...
void Parser::Dump2(FILE *fp)
{
	size_t i;

	for(i = 0; i < typeLibrary->globalTypeDeclList.size(); i++)
	{
