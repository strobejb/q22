...
bool Parser::ParseTags(Tag **tagList, TOKEN allowed[], bool allowTagSetUse)
{
	...
	while(t != ']')
	{
		...
		case TOK_TREE:
			...
			else if(tmp == TOK_OFFSET || tmp == TOK_COUNT || tmp == TOK_MAXCOUNT || tmp == TOK_TERMINATEDBY || tmp == TOK_ASSOC || tmp == TOK_OFFSETMAP)
			{
...
TagSet * Parser::ParseTagSet(FILEREF fileRef)
{
	...
	TOKEN allowed[] =
	{
		...
		TOK_ENUM, TOK_ENTRYPOINT, TOK_EXTENT, TOK_OPTIONAL, TOK_EXPORT, TOK_ASSOC, TOK_CATEGORY, TOK_CODE, TOK_MAGIC, TOK_OFFSETMAP, TOK_OPENAS, TOK_VERSION,
